...
import numpy as np
import os
import datetime  # Import datetime module to generate a unique timestamp
import logging
# Create a unique filename using the current date and time
log_filename = f"training_logs_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"

# Configure logging
logging.basicConfig(
    filename=log_filename,  # Use the unique filename
    level=logging.INFO,  # Log level
    format='%(asctime)s - %(levelname)s - %(message)s',  # Log message format
    filemode='w'  # Overwrite the file every time the script is run
)
...
#             predicted_captions = model.generate(images,max_length,tokenizer)
            ...
            for pc_idx,pc in enumerate(predicted_captions_decoded):
                ...
                logger.info(f"{pc_idx} - Target captions:\n {target_captions_decoded[pc_idx]}  \n{pc_idx} - predicted_captions:\n {pc} ")
    ...
    for epoch in range(100000):
        for batch_idx, (images, target_captions) in enumerate(train_loader):
            ...
            if target_captions.shape[1] >= max_token_filter:
                ...
                logger.info(f"Batch skipped as captions too long.")
                continue
            images = {'pixel_values': images.to(device)}
            target_captions = target_captions.to(device)
...
#             print ("Mean Loss",type(loss))
            ...
            if (step % log_step == 0):
                if step == 0:
                    ...
                    logger.info(f"Step {step}/{max_steps}: Avg Running Loss = {running_loss}")
                    
                else:
                    print(f"Step {step}/{max_steps}: Avg Running Loss = {running_loss /log_step}")
                    logger.info(f"Step {step}/{max_steps}: Avg Running Loss = {running_loss /log_step}")
                running_loss = 0.
